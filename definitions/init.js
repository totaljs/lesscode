...
CONF.ui && ROUTE('GET /flowstreams/', function($) {
	...
	if ($.xhr || $.query.type === 'json') {
		...
		for (let key in Flow.instances) {
			...
			let newstats = stats ? { memory: stats.memory ? (stats.memory / 1024 / 1024).floor(2) : 0, messages: stats.messages, pending: stats.pending, minutes: stats.minutes, paused: stats.paused, mm: stats.mm, errors: stats.errors } : EMPTYOBJECT;
